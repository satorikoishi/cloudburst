...
def create_dag(cloudburst_client):
    ...
    ## Hot: access same key
    def profile_executor(cloudburst, start_key, access_count, hot = False, client_name=DEFAULT_CLIENT_NAME):
        if hot:
    ...
    cloud_profile_executor = cloudburst_client.register(profile_executor, dag_name)
    if cloud_profile_executor:
...
def run(cloudburst_client, num_requests, sckt, args):
    ...
    logging.info(f'Running profile_executor, kvs_name {client_name}')
    ...
    utils.print_latency_stats(epoch_latencies, 'EPOCH %d E2E' %
                                (epoch), True, bname='profile_executor', args=args, csv_filename='benchmark_lat.csv')
